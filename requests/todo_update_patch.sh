...
TIMESTAMP_PATCH=$(date +%s)
PATCH_TITLE="Tarefa Parcialmente Modificada PATCH ${TIMESTAMP_PATCH}"
PATCH_COMPLETED=true
DATA_PATCH_TODO="{\"title\":\"$PATCH_TITLE\",\"completed\":$PATCH_COMPLETED}"

echo "Atualizando (PATCH) tarefa com ID: $TODO_ID via Vercel URL"
echo "Dados para Patch: $DATA_PATCH_TODO"
run_todo_test "Atualizar Tarefa (PATCH Vercel)" -X PATCH "$BASE_URL_TODOS/$TODO_ID" \
  -H "Content-Type: application/json" \
  -d "$DATA_PATCH_TODO"

echo "====== FIM DO SCRIPT TODO 05 (Vercel) ======"
