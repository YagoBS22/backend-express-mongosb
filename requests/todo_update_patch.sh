...
echo "====== SCRIPT ATUALIZAR TAREFA (PATCH) ======"
echo ""
...
if [ ! -f "$TODO_ID_FILE" ]; then echo "ERRO: Arquivo de ID da tarefa '$TODO_ID_FILE' não encontrado."; read -n 1 -s -r -p "Sair..." && echo "" && exit 1; fi
TODO_ID=$(cat "$TODO_ID_FILE")
if [ -z "$TODO_ID" ]; then echo "ERRO: ID da tarefa vazio em '$TODO_ID_FILE'."; read -n 1 -s -r -p "Sair..." && echo "" && exit 1; fi
...
PATCH_TITLE="Tarefa Sutilmente Modificada PATCH"
PATCH_COMPLETED=false

DATA_PATCH_TODO="{\"title\":\"$PATCH_TITLE\",\"completed\":$PATCH_COMPLETED}"

echo "Atualizando (PATCH) tarefa com ID: $TODO_ID"
echo "Dados para Patch: $DATA_PATCH_TODO"
run_todo_test "Atualizar Tarefa (PATCH)" -X PATCH "$BASE_URL_TODOS/$TODO_ID" \
  -H "Content-Type: application/json" \
  -d "$DATA_PATCH_TODO"

echo "====== FIM DO SCRIPT TODO 05 ======"
