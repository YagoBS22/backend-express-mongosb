...
echo "====== SCRIPT BUSCAR TAREFA POR ID ======"
echo ""
...
if [ ! -f "$TODO_ID_FILE" ]; then echo "ERRO: Arquivo de ID da tarefa '$TODO_ID_FILE' não encontrado. Execute 'todo_01_create.sh' primeiro."; read -n 1 -s -r -p "Sair..." && echo "" && exit 1; fi
TODO_ID=$(cat "$TODO_ID_FILE")
if [ -z "$TODO_ID" ]; then echo "ERRO: ID da tarefa não encontrado ou vazio em '$TODO_ID_FILE'."; read -n 1 -s -r -p "Sair..." && echo "" && exit 1; fi
...
echo "Buscando tarefa com ID: $TODO_ID"
run_todo_test "Buscar Tarefa por ID" -X GET "$BASE_URL_TODOS/$TODO_ID"

echo "====== FIM DO SCRIPT TODO 03 ======"
