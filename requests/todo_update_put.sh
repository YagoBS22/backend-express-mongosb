...
UPDATED_TITLE="Tarefa SUPER Atualizada PUT ${TIMESTAMP_UPDATE}"
UPDATED_DESCRIPTION="Descrição completa e totalmente atualizada via PUT ${TIMESTAMP_UPDATE}."
UPDATED_COMPLETED=true

DATA_UPDATE_TODO="{\"title\":\"$UPDATED_TITLE\",\"description\":\"$UPDATED_DESCRIPTION\",\"completed\":$UPDATED_COMPLETED}"

echo "Atualizando (PUT) tarefa com ID: $TODO_ID"
echo "Novos Dados: $DATA_UPDATE_TODO"
run_todo_test "Atualizar Tarefa (PUT)" -X PUT "$BASE_URL_TODOS/$TODO_ID" \
  -H "Content-Type: application/json" \
  -d "$DATA_UPDATE_TODO"

echo "====== FIM DO SCRIPT TODO 04 ======"
